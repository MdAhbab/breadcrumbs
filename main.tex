...
inventories, certificates, and purchase orders. Buyers in Europe and North America do not trust that paperwork on its own, so
they send auditors instead. Factories pay for audit after audit, and the
documents behind them can still be edited afterwards. Nobody can prove a record
is the same record that existed last month.

Breadcrumbs is a shared system that fixes two problems at once. First, it makes a
company's own internal records provable without publishing them. A factory keeps
its files. Only a fingerprint of each file goes onto a permissioned blockchain,
which is a shared database that a group of named organisations runs together and
that nobody can quietly rewrite. Anyone with permission can then check that a
document is genuine and unchanged, without ever seeing its contents.

Second, Breadcrumbs lets many factories build one shared fraud detector without
handing over their data. This uses federated learning, where each company trains
on its own records and shares only the resulting model adjustments. Because
compliance rules and fraud patterns keep changing, the shared model must keep
learning. Machine learning models normally lose old skills when taught something
new, a failure called catastrophic forgetting. We prevent it with a memory of
past categories that contains no original records. We then add a rule enforced
by the blockchain itself. A new model version is only accepted if it has not
damaged what the network already knew. We call this the Continuity Gate. It is
specified in this report, not yet built, and it is the first thing our prototype
would construct.

In a simulation on invented data with six factories and three waves of new fraud
types, ordinary federated learning lost 42.0 percentage points of accuracy on
earlier categories. Our method lost 7.6, at 77.9 percent average accuracy
against 81.0 for pooling every factory's raw data centrally, which
confidentiality rules forbid. Chance is 50 percent. These are simulation numbers
on invented data, not measurements of a deployed system, and Section~\ref{sec:ablation} reports
two tests that went against us.
...
requested and the decision rests with the United Nations General Assembly.
Either way, the country must increasingly compete on proof rather than price.
European due diligence duties on large buyers begin in 2029. European
obligations for high-risk artificial intelligence, including automatic logging
so a system's behaviour can be traced, were deferred in July 2026 and now begin
on 2 December 2027. Breadcrumbs is built for those dates, and it supports
Sustainable Development Goals 5, 8, 9, 12, 16 and 17.
...
and LiFeChain already puts federated lifelong learning on a blockchain. The one
column nobody else fills is the last one.
...
``Gate on past tasks'' means a rule that blocks a new model version when it has
lost accuracy on earlier tasks, checked against benchmarks committed in advance.
A dash means the capability is absent, not merely weaker.}
\label{tab:market}
\scriptsize
\begin{tabularx}{\textwidth}{@{}l c c c c c Y@{}}
\toprule
\textbf{System} & \textbf{Ledger} & \textbf{Internal} & \textbf{Shared} &
\textbf{Learns} & \textbf{Gate on} & \textbf{Primary purpose} \\
 & & \textbf{records} & \textbf{model} & \textbf{over time} & \textbf{past tasks} & \\
\midrule
...
Sedex/SMETA, amfori BSCI & -- & partial & -- & -- & -- & Audit reports and sharing \\
...
LiFeChain \citep{chen2025lifechain} & yes & -- & yes & yes & -- & Blockchain for federated lifelong learning \\
FedWeIT \citep{yoon2021fedweit} & -- & -- & yes & yes & -- & Federated continual learning method \\
Model registries, MLOps gates & -- & -- & -- & yes & partial & Accuracy gate, but single-owner and not pre-committed \\
\midrule
\textbf{Breadcrumbs} & yes & yes & yes & yes & yes & Verifiable internal records plus a
shared detector whose promotion rule is enforced on-chain \\
\bottomrule
\end{tabularx}
\end{table}
...
% =============================================================================
\section{Our two mechanisms, and one we removed}
\label{sec:novel}

The architecture above is a sound combination of known parts. What follows is
what we believe is new. An earlier version of this report described three
mechanisms. We tested them separately, found that one of the three made the
system worse, and removed it. Section~\ref{sec:ablation} gives the measurement.
...
the ledger. We have not implemented or measured this gate. It is specified here
and it is the first thing the prototype in Section~\ref{sec:proto} would build.
...
We propose a fourteen-week build producing a working demonstration with invented
data. Nothing in it requires a real factory to participate, which means the
demonstration can be shown to a factory before asking for anything.

The network runs three organisations on one machine using Fabric's test network:
a factory organisation with peers for six invented factories in Gazipur, Ashulia,
Narayanganj, Savar, Chattogram and Mirpur; a buyer organisation; and an audit
organisation. Ordering uses Raft. A generator produces about 50{,}000 invented
documents across five types, seeded with realistic errors: arithmetic that does
not add up, certificates whose identifiers fail a checksum, overtime exceeding
legal limits, and back-dated entries. Roughly 4 percent are planted anomalies,
and the generator writes a ground-truth label file so detection can be scored.

Federated learning uses Flower \citep{beutel2020flower} with the aggregator
reading and writing Fabric through the Gateway software development kit. The
demonstration script performs a full cycle. It commits documents, runs three
learning stages, and submits one candidate model that improves everything. Then
it submits a second candidate that deliberately damages an earlier task, so the
audience can watch the Continuity Gate reject it on-chain. That rejection is the
moment worth showing.

\begin{table}[htbp]
\centering
\caption{Fourteen-week prototype plan. Effort assumes three final-year students
working part time.}
\label{tab:plan}
\small
\begin{tabularx}{\textwidth}{@{}l l Y@{}}
\toprule
\textbf{Weeks} & \textbf{Deliverable} & \textbf{What must work at the end} \\
\midrule
1--2 & Network up & Three organisations, two channels, Raft ordering, certificates issued. \\
3--4 & \texttt{doccustody} & A document is hashed, stored encrypted, committed, and independently verified. \\
5 & Merkle disclosure & One line of a wage register is proved without revealing the others. \\
6--7 & Data generator & 50{,}000 labelled invented documents with planted anomalies. \\
8--9 & Federated baseline & Six clients train a shared detector; no raw data moves. \\
10 & Privacy layer & Update clipping, noise addition, trimmed-mean aggregation. \\
11--12 & Continual learning & Prototype replay with memory-bank hashes committed on-chain. \\
13 & \texttt{fedmodel} gate & Chaincode promotes a good model and rejects a damaging one. \\
14 & Demonstration & Web view, six-minute scripted walkthrough, public repository. \\
\bottomrule
\end{tabularx}
\end{table}
...
A fuller record of the hostile review this proposal was put through, including
the findings that changed the method and the two tests that went against us, is
kept in the companion file \texttt{REVIEW.md}.
...
\item The Continuity Gate, our main contribution, is \textbf{specified but not
built}. It appears in no line of code we ran, and no accuracy number here
involves a ledger.
\item Our benchmark cannot validate the learning claim
(Section~\ref{sec:ablation}). Only real documents can settle it.
\item We removed the Fisher penalty after measuring it, so the method changed
during the work.
\item We chose the difficulty setting, and it sits near the value that makes the
baseline look worst. Above a separation of about 1.00 the problem largely
...
\item The simulation omits every privacy and robustness component the design
...
\item The first-mile problem is mitigated, not solved: a signed record of a false
statement is still a correct record of a lie.
...
The part we consider new is what happens when that detector has to keep
learning. We anchor the continual learning memory on the ledger and put the promotion
decision inside a smart contract. That makes ``the shared model will not get
worse at what it already knew'' a rule the consortium enforces, rather than a
claim a vendor makes.

We separate what we measured from what we designed. The simulation contains no
ledger and no Continuity Gate. It shows the learning mechanism behaving as
intended on synthetic data: replay cut forgetting from 42.0 points to 7.6 at
77.9 percent average accuracy, against 81.0 for pooling raw data and 50.0 for
guessing. The gate is specified, not evaluated. We would rather say that than
let a reader think the blockchain produced the accuracy numbers.

The system is buildable in fourteen weeks with student resources, useful to a
single factory on day one, and complementary to the Digital Product Passport
work Bangladesh's industry body has already begun. That combination is why we
think it can actually be adopted, which is the only test that finally matters.
